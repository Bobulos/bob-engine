...
pub fn check_if_broken(connector: &mut PhysicsConnector, rb: &RigidBody) {
    if rb.force.length_sq() >= connector.cxn_strength_ln_sq
        || rb.torque >= connector.cxn_strength_ang
    {
...
impl SystemBase for ConnectorSystem {
    ...
    fn on_update(&mut self, world: &Arc<DynamicWorld>) {
        world.for_each2_mut_both::<RigidBody, PhysicsConnector>(
            |entity, rb, joint| {
