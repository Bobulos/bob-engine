...
impl TilemapRenderer {
    ...
    pub fn update(
        ...
        self.bind_group = Some(device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: self.tile_uv_buffer.as_ref().unwrap().as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
                    resource: self.info_buffer.as_ref().unwrap().as_entire_binding(),
                },
            ],
        }));
    ...
    pub fn record(&self, rpass: &mut wgpu::RenderPass) {
        let (Some(bg), w, h) = (&self.bind_group, self.width, self.height) else {
            return;
        };
        if w == 0 || h == 0 {
            return;
